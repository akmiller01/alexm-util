...
parser = OptionParser()
parser.add_option("-i", "--input", dest="input", default="D:\\Documents\\Data\\Moroto budget\\2017\\Moroto BFP.pdf",
                help="Input pdf name", metavar="FILE")
parser.add_option("-o", "--output", dest="output", default="D:\\Documents\\Data\\Moroto budget\\2017\\",
                        help="Output path. Default is 'D:\\Documents\\Data\\Moroto budget\\'",metavar="FOLDER")
...
def expenditure():
    ...
    if options.debug:
        pdb.set_trace()
    keys = output[0].keys()
    with open(options.output+"Expenditure.csv", 'wb') as output_file:
...
def revenue():
    ...
    if options.debug:
        pdb.set_trace()
    keys = output[0].keys()
    with open(options.output+"Revenue.csv", 'wb') as output_file:
        dict_writer = csv.DictWriter(output_file, keys)
        dict_writer.writeheader()
        dict_writer.writerows(output)
    sys.stdout.write("\n")
