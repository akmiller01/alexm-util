...
files <- list.files(wd,pattern="..hr.....RData",full.names=TRUE)
...
for(i in 1:length(files)){
  file <- files[i]
  cc <- substr(basename(file),1,2)
  phase <- substr(basename(file),5,6)
  ...
  load(paste0(cc,"pr",phase,"fl.RData"))
  
  pr <- data
  
  if(!"hv271" %in% names(pr)){
    load(paste0(cc,"wi",phase,"fl.RData"))
    names(data) <- c("hhid","wealth","hv270")
    pr <- join(data,pr,by="hhid")
  }else{
...
povcalcuts <- read.csv("headcounts2.csv",as.is=TRUE)
...
repeats <- c("cf31","dr61","gy5i","kk42","ky61","mb53","np60","st50","sz51","ua51","vn52","Zimbabwe_MICS5_Datasets")

total <- subset(total,!(filename) %in% repeats)