...
for(i in 2:length(dirs)){
  ...
  if(basename(dir) %in% povcalcuts$filename){
    ...
    recode.educ <- function(x){
      ...
      else if(x==0 | x==1 | tolower(x)=="no education, pre-primary" tolower(x)=="no education, preschool" | tolower(x)=="no education" | tolower(x)=="incomplete primary" | tolower(x)=="primary incomplete"){return("No education, preschool")}
      else if(x==2 | x==3 | tolower(x)=="fundamental (preparatory, unified)" | tolower(x)=="diploma before secondary" | tolower(x)=="primary" | tolower(x)=="complete primary" | tolower(x)=="primary complete" | tolower(x)=="incomplete secondary" | tolower(x)=="secondary incomplete"){return("Primary")}
      else if(x==4 | tolower(x)=="secondary" | tolower(x)=="diploma after secondary" | tolower(x)=="complete secondary" | tolower(x)=="secondary complete"){return("Secondary")}
      else if(x==5 | tolower(x)=="higher" | tolower(x)=="university/higher"){return("Higher")}
      else{return(NA)}
    }
    ...
    mothers <- unique(pr[c("cluster","household","line","woman.bmi","educ")])
    names(mothers) <- c("cluster","household","mother.line","mother.bmi","mother.educ")
    pr <- join(
      pr
      ,mothers
      ,by=c("cluster","household","mother.line")
    )
    if(length(is.na(pr$mother.line))==length(pr$mother.line)){
      if(length(is.na(pr$hc61))==length(pr$hc61)){
        pr$mother.educ <- sapply(pr$hc68,recode.educ)
      }else{
        pr$mother.educ <- sapply(pr$hc61,recode.educ)
      }
    }
    ...
    keep <- c("wealth","weights","urban.rural","educ","age","sex","cluster","household","head.sex","head.age","p20"
              ...
              ,"woman.bmi","man.bmi","child.weights","mother.bmi","mother.educ"
    )
    prNames <- names(pr)
...
for(i in 2:length(dirs)){
  ...
  if(hrBase %in% povcalcuts$filename){
    ...
    hh <- data.frame(hh,check.names=FALSE)
    hl <- data.frame(hl,check.names=FALSE)
    ch <- data.frame(ch,check.names=FALSE)
    wm <- data.frame(wm,check.names=FALSE)
    ...
    #Mother educ descriptions are not the same...
    # hl$mother.educ <- recode.educ(rep(NA,length(hl$melevel)),hl$melevel,rep(NA,length(hl$melevel)))
    mothers <- unique(hl[c("cluster","household","line","educ")])
    names(mothers) <- c("cluster","household","mline","mother.educ")
    hl <- join(
      hl
      ,mothers
      ,by=c("cluster","household","mline")
    )
    ...
    povcalcut <- subset(povcalcuts,filename==hrBase)$hc
    povperc <- weighted.percentile(hh$wealth,hh$weights,prob=povcalcut)
    
    hh$p20 <- (hh$wealth < povperc)
...
data.total$mother.educ <- sapply(data.total$mother.educ,recode.educ)
data.total$mother.educ <- factor(data.total$mother.educ
                          ,levels = c("No education, preschool","Primary","Secondary","Higher")
)
...
names(pr)[which(names(pr)=="P005M_MAE")] <- "mother.line"
names(pr)[which(names(pr)=="DOMICILIO_ID")] <- "household"
pr$line.trunc <- as.integer(substr(pr$line,(nchar(pr$household)-nchar(pr$cluster))+1,nchar(pr$line)))
...
mothers <- unique(pr[c("cluster","household","line.trunc","educ")])
names(mothers) <- c("cluster","household","mother.line","mother.educ")
pr <- join(
  pr
  ,mothers
  ,by=c("cluster","household","mother.line")
...
keep <- c("wealth","weights","urban.rural","urban","educ","age","sex","cluster","household","head.sex","head.age","p20"
          ...
          ,"woman.bmi","man.bmi","ageCategory","head.ageCategory","stunting","mother.educ"
)
prNames <- names(pr)
namesDiff <- setdiff(keep,prNames)
if(length(namesDiff)>0){
...
ir$mother.educ <- sapply(ir$tb4_a12_m,recode.educ)
ir$mother.educ <- factor(ir$mother.educ
                  ,levels = c("No education, preschool","Primary","Secondary","Higher")
)
...
povperc <- weighted.percentile(ir$wealth,ir$weights,prob=povcalcut)
...
save(data.total,file="total_meduc.RData")
load("total_meduc.RData")
# mom.tab <- data.table(data.total)[,.(count=sum(!is.na(mother.educ)),educ=sum(!is.na(educ))),by=.(filename)]
data.total$mother.educ <- unfactor(data.total$mother.educ)
data.total <- data.frame(data.total)
data.total <- data.total[complete.cases(data.total[c("p20","weights","mother.educ")]),]
# maternal.educ <- data.table(school.age)[,.(no.educ=weighted.mean(mother.educ=="No education, preschool",weights,na.rm=TRUE),
#                                            primary=weighted.mean(mother.educ=="Primary",weights,na.rm=TRUE),
#                                            secondary=weighted.mean(mother.educ=="Secondary",weights,na.rm=TRUE),
#                                            higher=weighted.mean(mother.educ=="Higher",weights,na.rm=TRUE)
#                                            ),by=.(filename,p20)]
maternal.educ <- data.table(data.total)[,.(no.educ.stunted=weighted.mean(mother.educ=="No education, preschool" & stunting!="Not stunted",weights,na.rm=TRUE),
                                           educ.stunted=weighted.mean(mother.educ!="No education, preschool" & stunting!="Not stunted",weights,na.rm=TRUE),
                                           no.educ.not.stunted=weighted.mean(mother.educ=="No education, preschool" & stunting=="Not stunted",weights,na.rm=TRUE),
                                           educ.not.stunted=weighted.mean(mother.educ!="No education, preschool" & stunting=="Not stunted",weights,na.rm=TRUE)

),by=.(filename,p20)]
write.csv(maternal.educ,"maternal_educ.csv",row.names=FALSE,na="")
