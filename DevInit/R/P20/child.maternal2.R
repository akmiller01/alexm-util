...
pop <- povcalcuts[c("filename","hc","female.under5","male.under5")]
pop$under5 <- psum(pop$female.under5,pop$male.under5)

vacc.tab <- data.table(child.health)[,.(any.vacc=weighted.mean(vacc,weights,na.rm=TRUE)),by=.(filename,p20)]
vacc.tab <- vacc.tab[complete.cases(vacc.tab),]
vacc.tab <- merge(vacc.tab,pop,by="filename")
vacc.tab <- transform(vacc.tab,pop.segment =
                        p20*(hc*under5) +
                        (!p20)*((1-hc)*under5)
)
setwd("D:/Documents/Data/DHSmeta2")

write.csv(vacc.tab,"vacc-nationally.csv",na="",row.names=FALSE)

vacc.global <- vacc.tab[,.(any.vacc=weighted.mean(any.vacc,under5,na.rm=TRUE))]