...
for(i in 2:length(dirs)){
  ...
  if(basename(dir) %in% povcalcuts$filename){
    ...
    if(typeof(pr$child.height.age)=="NULL"){
      ...
      pr$child.height.age <- pr$child.height.age/1000
    }
...
missing.h4a <- c(9.997,9.998,9.999)
data.total$child.height.age[which(data.total$child.height.age %in% missing.h4a)] <- NA
...
pr$child.height.age <- pr$child.height.age/10
pr$stunting <- NA
...
ir$child.height.age <- ir$child.height.age/10
ir$stunting <- NA
...
wd <- "D:/Documents/P20 Visualizations"
setwd(wd)
